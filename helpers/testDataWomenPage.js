// URL

export const BASE_URL = 'https://magento.softwaretestingboard.com';
export const WOMEN_URL = 'https://magento.softwaretestingboard.com/women.html';
export const WOMEN_TOPS_PAGE_URL = 'https://magento.softwaretestingboard.com/women/tops-women.html';
export const WOMEN_BOTTOMS_PAGE_URL = 'https://magento.softwaretestingboard.com/women/bottoms-women.html';



//test data
export const WOMEN_HEADER_TEXT = 'Women';
export const MENU_TOPS_TEXT = 'Tops';
export const ITEM_HOODIES_SWEATSHIRTS_LINK_TEXT = 'Hoodies & Sweatshirts';
export const ITEM_JACKETS_LINK_TEXT = 'Jackets';
export const ITEM_TEES_LINK_TEXT = 'Tees';
export const ITEM_BRASTANKS_LINK_TEXT = 'Bras & Tanks';

export const TOPS_ITEM_LINKS = [
    'https://magento.softwaretestingboard.com/women/tops-women/hoodies-and-sweatshirts-women.html',
    'https://magento.softwaretestingboard.com/women/tops-women/jackets-women.html',
    'https://magento.softwaretestingboard.com/women/tops-women/tees-women.html',
    'https://magento.softwaretestingboard.com/women/tops-women/tanks-women.html'
];

export const MENU_BOTTOMS_TEXT = 'Bottoms';
export const ITEM_PANTS_LINK_TEXT = 'Pants';
export const ITEM_SHORTS_LINK_TEXT = 'Shorts';

export const BOTTOMS_ITEM_LINKS = [
    'https://magento.softwaretestingboard.com/women/bottoms-women/pants-women.html',
    'https://magento.softwaretestingboard.com/women/bottoms-women/shorts-women.html'
];
export const PROMO_BLOCK_INFO_TEXT = 'New Luma Yoga Collection';
export const PROMO_BLOCK_INFO_TITLE_TEXT = 'Yoga is ancientClothing shouldn’t be';
export const BLOCK_CONTENT_TEES_INFO_TEXT = 'You can’t have too many tees';
export const BLOCK_CONTENT_TEES_ACTION_TEXT = '4 tees for the price of 3. Right now';
export const BLOCK_20_OFF_HEADER_TEXT = 'Hot pantsHot deals';
export const BLOCK_20_OFF_ACTION_TEXT = '20% OFFLuma pants when you shop today*';
export const COMPARE_PRODUCTS_HEADING = 'Compare Products';
export const WISH_LIST_HEADING = 'My Wish List';
export const ERIN_RECOMMENDS_HEADING = 'What would Erin wear?';
export const ERIN_RECOMMENDS_ACTION_TEXT = 'It’s no secret: see Luma founder Erin Renny’s wardrobe go-to’s';
export const BLOCK_PANTS_HEADING = 'Luma pants';
export const BLOCK_PANTS_ACTION = 'Pants for yoga, gym and outdoors';
export const BLOCK_SHOP_NOW_HEADING = 'Luma BrasTanks';
export const BLOCK_SHOP_NOW_Action = 'Stock up for summer!';